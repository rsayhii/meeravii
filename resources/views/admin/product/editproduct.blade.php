...
<script>
    ...
    document.getElementById('add_variant').addEventListener('click', () => {
        ...
        <div class="border p-3 mb-3 rounded-md variant-item">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-2">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Sizes (JSON)</label>
                    <textarea name="variants[${idx}][sizes]" class="w-full border border-gray-300 rounded-md px-2 py-1" placeholder='[{"size":"S","qty":10}]' required></textarea>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Images</label>
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('remove-product-image')) {
            const parent = e.target.closest('.product-image-item');
            const path = parent.dataset.path;
            document.getElementById('removed_product_inputs').insertAdjacentHTML('beforeend', `<input type="hidden" name="removed_images[]" value="${path}">`);
            parent.remove();
        }
    });
    ...
    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('remove-variant-image')) {
            const parent = e.target.closest('.variant-image-item');
            const path = parent.dataset.path;
            const idx = parent.dataset.variantIndex;
            document.getElementById('removed_inputs').insertAdjacentHTML('beforeend', `<input type="hidden" name="removed_variants[${idx}][]" value="${path}">`);
            parent.remove();
        }
    });
</script>
